    ...
    const draw = () => {
      ...
      localNodes.forEach(node => {
        node.connections.forEach(connId => {
          ...
          ctx.strokeStyle = risk > 70 ? 'rgba(255, 0, 64, 0.2)' : risk > 40 ? 'rgba(255, 149, 0, 0.15)' : 'rgba(0, 240, 255, 0.1)';
      ...
      // Draw nodes
      localNodes.forEach(node => {
        const color = node.isCompromised ? '#ff0040' : typeColors[node.type] || '#64748b';
        ...
        if (node.isCompromised || node.riskScore > 60) {
          ...
          ctx.fillStyle = node.isCompromised ? 'rgba(255, 0, 64, 0.15)' : 'rgba(0, 240, 255, 0.1)';
          ctx.fill();
        }
    ...
    <div className="p-4 space-y-3 h-full flex flex-col">
      ...
      <div className="grid grid-cols-5 gap-3">
        {[
          ...
          { label: 'CONNECTIONS', value: totalConnections, color: 'text-primary' },
          { label: 'COMPROMISED', value: compromised, color: 'text-destructive' },
          { label: 'AVG RISK', value: avgRisk, color: avgRisk > 50 ? 'text-warning' : 'text-success' },
          { label: 'SEGMENTS', value: 6, color: 'text-primary' },
        ].map(s => (
          <div key={s.label} className="bg-card/60 border border-border rounded-sm px-4 py-2 text-center">
            <div className="text-[10px] font-mono text-muted-foreground">{s.label}</div>
            <AnimatedCounter value={s.value} className={`text-lg font-bold ${s.color}`} />
          </div>
        ))}
      </div>
      ...
      <div className="flex gap-3 flex-1 min-h-0">
        <GlowCard
          ...
          header={
            <>
              ...
              <span className="text-[13px] font-mono font-medium text-foreground">NETWORK TOPOLOGY — GNN VISUALIZATION</span>
            </>
          }
        >
          <canvas
        ...
        {selectedNode && (
          <GlowCard
            ...
            header={
              <>
                <PulsingDot color={selectedNode.isCompromised ? 'red' : 'green'} />
                <span className="text-[13px] font-mono font-medium text-foreground">NODE DETAILS</span>
              </>
            }
          >
            <div className="space-y-3 text-[12px] font-mono">
              <div>
                ...
                <span className="ml-2 text-foreground">{selectedNode.hostname}</span>
              </div>
              <div>
                <span className="text-muted-foreground">IP:</span>
                <span className="ml-2 text-foreground tabular-nums">{selectedNode.ip}</span>
              </div>
              <div>
                <span className="text-muted-foreground">Type:</span>
                <span className="ml-2"><StatusBadge severity={selectedNode.isCompromised ? 'critical' : 'info'}>{selectedNode.type}</StatusBadge></span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className={`ml-2 font-bold ${selectedNode.riskScore > 70 ? 'text-destructive' : selectedNode.riskScore > 40 ? 'text-warning' : 'text-success'}`}>
              ...
              <div>
                ...
                <div className="mt-1 flex flex-wrap gap-1">
                  {selectedNode.openPorts.map(p => (
                    <span key={p} className="px-1.5 py-0.5 bg-primary/10 text-primary text-[10px] rounded-sm border border-primary/20">
                      {p}
                    </span>
                  ))}
                </div>
